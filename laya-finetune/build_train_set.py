"""Build the TRAINING corpus in two resumable stages, the same way the test set was built.

    stage 1  generate (question, answer) with a planted defect  -> data/train_generated.jsonl
    stage 2  gold-label each one blind                          -> data/train_judged.jsonl

Ids start at ID_OFFSET so no training id can collide with the 500 test ids (0-499). The
generator, prompts and reference judge are unchanged from the test-set build, so training
and test items come from one distribution.
    ...
    for attempt in range(tries):
        try:
            ...
            if err.status_code not in (429, 500, 502, 503, 529) or attempt == tries - 1:
                print(f"{model}: {err.status_code} {err.message[:160]}", flush=True)
                return None, {}
...
async def generate(client: AsyncAnthropic, target: int, concurrency: int = 10) -> None:
    ...
    todo = [i for i in range(ID_OFFSET, ID_OFFSET + target) if i not in have]
