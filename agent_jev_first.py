...
On a MISSING reply the run falls back to agent_jev.py, the arm where Jev predicts lookups while Claude keeps its tools."""
...
from agent_claude import MAX_TOKENS, MODEL, RunResult
from agent_jev import SpecStats, candidate_calls, question, run_agent_with_jev
...
async def gather_facts(task: str, jev: AsyncTypeSafeClient, stats: SpecStats) -> list[str]:
...
async def run_agent_jev_first(task: str, model: str = MODEL) -> tuple[RunResult, SpecStats, bool]:
    ...
    stats.hits = result.tool_calls = len(facts)
    ...
    if fell_back:
        fallback, fallback_stats = await run_agent_with_jev(task, model)
        ...
        stats.misses, stats.jev_calls = fallback_stats.misses, stats.jev_calls + fallback_stats.jev_calls
        stats.jev_tokens += fallback_stats.jev_tokens
    result.stop()
